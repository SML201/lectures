...
The goal of dimensionality reduction is to extract low dimensional representations of high dimensional data that are useful for visualization, exploration, inference, or predicition.
...
One this is found, then a least squares linear regression of each $\pmb{x}_i$ on $\tilde{\pmb{x}}$ (with no intercept) is performed and the residuals are obtained.  The next principal component (PC2) is then calculated using the same procedure. 

This process is repeated up to min$(p, n)$ times.
...
- `scaled`: if `TRUE` then the variables are std dev scaled before calculating PCs

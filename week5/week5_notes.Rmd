...
opts_chunk$set(fig.align="center", fig.height=3, fig.width=4.5, 
...
The `geom_boxplot()` layer forms a boxplot and only requires both `x` and `y` assignments in the `aes()` call, even when plotting a single boxplot:
...
We can create a histogram using the `geom_histogram()` layer, whih requires an `x` argument only in the `aes()` call:
